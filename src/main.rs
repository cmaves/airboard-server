...
fn get_clipboard() -> Result<Vec<u8>, std::io::Error> {
	...
	if out.status.success() {
		...
		Err(std::io::Error::new(std::io::ErrorKind::Other, "Non-zero return"))
	}
}
...
fn main() {
    ...
    */
    ...
    if let Err(_) = blue.start_advertise(adv) {
...
impl OutSyncer {
    ...
    fn read_fn(&self) -> ([u8; 512], usize) {
        if self.cur_pos == std::usize::MAX {
            ...
            let end = self.buf.len().min(self.cur_pos + 512 - 8);
            let mut v = [0; 512];
